<template>
  <div>
    <div v-if="showList">
      <div>
        <el-select v-model="selectType" placeholder="项目类型">
          <el-option
            v-for="(projectType,indexj) in projectTypeList"
            :label="projectType.name"
            :value="projectType.urid"
            :key="indexj"
          ></el-option>
        </el-select>
      </div>
    ...
    <div v-if="showDetail">
      <div v-if="showBtnGroup">
        ...
        <el-button type="primary" size="small" @click="confirm" v-if="showSupplier">我要投标</el-button>
      </div>
      <div v-if="showMain">
        <div style="width: 100% ;">
              ...
              <el-col :span="8">
                <el-form-item label="项目类型" class="label" prop="typeid">
                  <el-input v-model="registerForm.typeid" placeholder="项目类型" class="inputInfo"
                            :disabled="isEdit" style="width: 250px">
                  </el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
  ...
  export default {
    ...
    methods: {
      ...
      initDictionary() {
        ...
        this.projectForm.forEach(project => {
          //项目类型赋值
          this.projectTypeList.forEach(projectType => {
            if (project.typeid == projectType.urid) {
              project.typeid = projectType.name
            }
          })
          if (project.invitationtype == 1) {
            project.invitationtype = '公开招标'
          } else {
            project.invitationtype = '邀标'
          }
        })
      }
